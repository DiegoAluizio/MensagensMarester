...
# obtendo saudacao condizente com o horario de brasilia
def obter_saudacao():
    fuso_brasilia = datetime.timezone(datetime.timedelta(hours=-3))
    hora_atual = datetime.datetime.now(fuso_brasilia).hour
...
with col2:
    parcela = st.number_input("Número da Parcela", min_value=1, max_value=12, value=1)
...
# exibicao da mensagem gerada
st.divider()
st.subheader("Mensagem Gerada:")

if mensagem_final:
    st.code(mensagem_final, language="text")
    st.info("Para copiar a mensagem, basta passar o mouse sobre o quadro acima e clicar no ícone de cópia no canto superior direito.")
else:
